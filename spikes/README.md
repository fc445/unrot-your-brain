...
| 20260926 | [PR-35](https://linear.app/freddie-cassidy/issue/PR-35/spike-can-sparkle-auto-update-the-mac-app-while-builds-are-signed-ad) | [sparkle-adhoc-updates](20260926-PR-35-sparkle-adhoc-updates/) | Sparkle 2.10 self-updates an ad-hoc-signed app end to end (1→2 prod, 1→3 dev, tampered signature refused): EdDSA alone is enough, and `<sparkle:channel>` keeps dev items away from prod. The catch is that every update changes the cdhash, so the login-keychain API key prompts again after each update until there is a Developer ID or the key moves. |
